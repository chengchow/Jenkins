...
# Usage: init.sh -d jenkins转存目录 -e (prod|dev) -k SSH秘钥 -n 项目名称 -t (war|jar) -j 项目端口 -x SSH主机名 -u SSH用户 -p SSH端口 -h 项目路径 -v 项目用户 -D 启动脚本git地址 -C 配置文件git地址
# Usage: publish.sh -d jenkins转存目录 -e (prod|dev) -k SSH秘钥 -n 项目名称 -t (war|jar) -u SSH用户 -x SSH主机名 -p SSH端口 -h 项目路径 -v 项目用户

help_doc="Usage: `basename $0` 
[ -d jenkins转存目录 ] -e (prod|dev) [ -k SSH秘钥 ] -n 项目名称 -t (war|jar) -j 项目端口 -x 后端服务器1 -y 后端服务器2 [ -u 后端SSH用户 ] [ -p 后端SSH端口 ] [ -h 项目路径 ] [ -H 项目路径 ] [ -v 项目用户 ] -Z Nginx服务器 -i Nginx服务器 [ -U Nginx SSH用户 ] [ -P Nginx SSH端口 ] [ -F Nginx Upstream文件位置 ] [ -c Nginx Upstream文件位置 ] [ -L 集群标签 ] [ -l 集群标签 ] [ -D daemon文件git地址 ] -C 环境配置git地址
"
...
## 规范必选变量
if [ -z $PROJ_NAME ] || [ -z $PROJ_TYPE ] || [ -z $PROJ_ENV ] || [ -z $NGX_SSH_HOST ] || [ -z $PROJ_PORT ] || [ -z $WEB_SSH_HOST1 ] || [ -z $WEB_SSH_HOST2 ] || [ -z $CONFIG_GIT_ADDR ];then
    echo $help_doc
    exit 101
fi
...
DEFEALT_DAEMON_GIT_ADDR="git@10.10.21.100:operations/conf/daemon.git"
...
## 编译代码
if [ -f $POM_FILE ];then
    $SCRIPTS_PATH/build.sh -f $POM_FILE
    comm_exec `echo $?`
fi
...
if [ "$(echo $WEB_SSH_HOST1 | tr A-Z a-z)" != "none" ];then
    ...
    ## 后端1项目初始化
    for i in $(echo $WEB_SSH_HOST);do
        $SCRIPTS_PATH/init.sh -d $JKS_ROOT -e $PROJ_ENV -k $WEB_SSH_KEY -n $PROJ_NAME -t $PROJ_TYPE -j $PROJ_PORT -x $i -u $WEB_SSH_USER -p $WEB_SSH_PORT -h $PUBLISH_PATH -v $PROJ_USER -D $DAEMON_GIT_ADDR -C $CONFIG_GIT_ADDR
        comm_exec `echo $?`
    done
    ...
    ## 正在发布后端1服务
    for i in $(echo $WEB_SSH_HOST);do
        $SCRIPTS_PATH/publish.sh -d $JKS_ROOT -e $PROJ_ENV -k $WEB_SSH_KEY -n $PROJ_NAME -t $PROJ_TYPE -u $WEB_SSH_USER -x $i -p $WEB_SSH_PORT -h $PUBLISH_PATH -v $PROJ_USER
        comm_exec `echo $?`
    done
